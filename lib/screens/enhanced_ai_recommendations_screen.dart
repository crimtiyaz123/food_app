...
class EnhancedAIRecommendationsScreen extends StatefulWidget {
  final String userId;

  const EnhancedAIRecommendationsScreen({super.key, required this.userId});

  @override
  State<EnhancedAIRecommendationsScreen> createState() => _EnhancedAIRecommendationsScreenState();
}

class _EnhancedAIRecommendationsScreenState extends State<EnhancedAIRecommendationsScreen>
    with TickerProviderStateMixin {
  late TabController _tabController;
  final EnhancedAIRecommendationService _recommendationService = EnhancedAIRecommendationService();

  // Different types of recommendations
  final List<Product> _personalizedRecommendations = [];
  final List<Product> _contextualRecommendations = [];
  final List<Product> _trendingRecommendations = [];
  final List<Product> _similarProducts = [];

  bool _isLoading = true;
  final String _selectedContext = 'lunch';
  ...
  Widget _buildAIInsightsTab() {
    return _buildPersonalizedInsights();
  }

  Widget _buildPersonalizedInsights() {
    if (_personalizedRecommendations.isEmpty) {
      return _buildEmptyState('AI Insights', 'Start ordering to get personalized insights');
    }
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(Icons.lightbulb, color: AppColors.primaryRed, size: 20),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('Personalized recommendations for you', style: AppTextStyles.bodySmall.copyWith(
          ...
          Expanded(
            child: GridView.builder(
              ...
              itemBuilder: (context, index) {
                return _buildProductCard(_personalizedRecommendations[index]);
              },
            ),
          ),
        ],
  ...
  Widget _buildContextChip(String label, String value, IconData icon) {
    ...
    return GestureDetector(
      onTap: () {
        setState(() {
          // Update selected context logic here
        });
        _loadRecommendations();
      },
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Advanced AI-powered food suggestions',
  ...
  Widget _buildProductCard(Product product) {
    return GestureDetector(
      onTap: () async {
        ...
        // Navigate to product detail (using a simple dialog since ProductScreen needs category)
        if (mounted) {
          showDialog(
            context: context,
