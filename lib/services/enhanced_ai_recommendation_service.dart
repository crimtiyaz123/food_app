...
/// Enhanced AI Recommendation Service with Advanced ML Algorithms
/// Implements collaborative filtering, content-based filtering, and contextual recommendations
class EnhancedAIRecommendationService {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;

  // Get comprehensive personalized recommendations using ML algorithms
  Future<List<Product>> getPersonalizedRecommendations({
    ...
    try {
      ...
      final validProducts = allProducts.where((p) => p.id != null).toList();
  ...
  Future<List<app_order.Order>> _getUserOrderHistory(String userId) async {
    ...
    return snapshot.docs.map((doc) {
      ...
      return app_order.Order(
        id: doc.id,
        product: Product.fromFirestore(data['productId'] ?? '', data),
  ...
  double _calculateUserSimilarity(UserPreferences user1, UserPreferences user2) {
    ...
    final commonCategories = user1.favoriteCategories
        ...
        .length;
    if (user1.favoriteCategories.isNotEmpty && user2.favoriteCategories.isNotEmpty) {
      similarity += commonCategories / (user1.favoriteCategories.length + user2.favoriteCategories.length);
      factors++;
    }
  ...
  Future<List<Product>> _generateAdvancedRecommendations({
    ...
    final contentBasedScores = _getEnhancedContentBasedRecommendations(
      allProducts, userPreferences, orderHistory);
    
    // 2. Collaborative Filtering
    final collaborativeScores = _getEnhancedCollaborativeRecommendations(
      allProducts, similarUsers, orderHistory);
    
    // 3. Contextual Recommendations
    final contextualScores = _getContextualRecommendationsML(
      allProducts, userContext, context);
    
    // 4. Trending Products
    final trendingScores = _getTrendingRecommendationsML(allProducts);
    
    // 5. Popularity-based (global trends)
    final popularityScores = _getPopularityRecommendationsML(allProducts);
    ...
    for (final product in allProducts) {
      ...
      totalScore += (contentBasedScores[product.id] ?? 0.0) * 0.3;
      totalScore += (collaborativeScores[product.id] ?? 0.0) * 0.25;
      totalScore += (contextualScores[product.id] ?? 0.0) * 0.2;
      totalScore += (trendingScores[product.id] ?? 0.0) * 0.15;
      totalScore += (popularityScores[product.id] ?? 0.0) * 0.1;
      ...
      final recentOrder = orderHistory.where((order) => 
        order.product.id == product.id).toList();
      ...
      productScores[product.id!] = totalScore;
    }
    ...
    for (var entry in sortedProducts.take(limit)) {
      final product = allProducts.firstWhere((p) => p.id == entry.key, orElse: () {
        // Fallback: return a default product if not found
  ...
  Map<String, double> _getEnhancedContentBasedRecommendations(
    List<Product> products, UserPreferences preferences, List<app_order.Order> orderHistory) {
  ...
  // Enhanced collaborative recommendations
  Map<String, double> _getEnhancedCollaborativeRecommendations(
    ...
    // Count how many similar users have each product in their preferences
    for (final user in similarUsers) {
      for (final category in user.favoriteCategories) {
        ...
        for (final product in productsInCategory) {
          productPopularity[product.id!] = (productPopularity[product.id!] ?? 0) + 1;
        }
      }
    }
    
    // Normalize scores
    final maxPopularity = productPopularity.values.isEmpty ? 1 : productPopularity.values.reduce(math.max);
    for (final entry in productPopularity.entries) {
      scores[entry.key] = entry.value / maxPopularity;
    }
    
    return scores;
  }

  // Contextual recommendations with ML
  Map<String, double> _getContextualRecommendationsML(
  ...
  // ML-based trending recommendations
  Map<String, double> _getTrendingRecommendationsML(List<Product> products) {
    final scores = <String, double>{};
    ...
    for (final product in products) {
      ...
      // Assuming products with currentPrice != basePrice are trending due to demand
  ...
  // ML-based popularity recommendations
  Map<String, double> _getPopularityRecommendationsML(List<Product> products) {
    final scores = <String, double>{};
    ...
    for (final product in products) {
      ...
      // Mock review count since it's not in the Product model
      final mockReviewCount = product.rating != null ? (product.rating! * 10).toInt() : 0;
      if (mockReviewCount > 0) {
        final normalizedReviews = math.min(mockReviewCount / 100.0, 1.0);
  ...
  int _countCommonItems(List<String> set1, List<String> set2) {
    return set1.where((item) => set2.contains(item)).length;
  }

  int _countTotalItems(List<String> set1, List<String> set2) {
  ...
  List<Product> _filterByLocation(List<Product> products, String location) {
    ...
    return products.take(products.length ~/ 2).toList();
  }
  ...
  List<Product> _sortBySimilarity(List<Product> products, Product targetProduct) {
    ...
    for (final product in products) {
      final similarity = _calculateProductSimilarity(product, targetProduct);
  ...
  double _calculateProductSimilarity(Product product1, Product product2) {
    ...
    final priceSimilarity = 1.0 / (1.0 + priceDiff);
    ...
    if (product1.rating != null && product2.rating != null) {
      ...
      final ratingSimilarity = 1.0 / (1.0 + ratingDiff);
